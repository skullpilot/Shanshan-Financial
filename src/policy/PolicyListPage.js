...
function PolicyListPage({ policies, customers }) {
  ...
  const policiesWithNames = policies.map((policy) => {
    const insuredLegalName = `${customers[policy.insuredId].firstName}, ${
      customers[policy.insuredId].lastName
    }`;
  ...
  function filterPolicyByName(policiesWithNames, customers, val) {
    ...
    const filteredPolicies = Lodash.filter(policiesWithNames, (policy) => {
      ...
      const matchInsured =
        (insured.name && insured.name.toLowerCase().includes(input)) ||
        policy.insuredLegalName.toLowerCase().includes(input);

      const matchOwner =
        (owner.name && owner.name.toLowerCase().includes(input)) ||
        policy.ownerLegalName.toLowerCase().includes(input);
  ...
  function filterPolicyByMonth(policiesWithNames, val) {
    return policiesWithNames.filter(
      (policy) => policy.policyDate && policy.policyDate.split("-")[1] === val
    );
  }

  function filterPolicyByYear(policiesWithNames, val) {
    return policiesWithNames.filter(
      (policy) => policy.policyDate && policy.policyDate.split("-")[0] === val
    );
  }
  ...
  const searchCustomerValue =
    searchContent.searchType === "CUSTOMER_NAME" && searchContent.searchValue
      ...
      : "";
  const searchMonthValue =
    searchContent.searchType === "MONTH" && searchContent.searchValue
      ? searchContent.searchValue
      : "";
  const searchYearValue =
    searchContent.searchType === "YEAR" && searchContent.searchValue
      ? searchContent.searchValue
      : "";
  const searchPlanValue =
    searchContent.searchType === "PLAN" && searchContent.searchValue
      ? searchContent.searchValue
      : "";
  const searchCompanyValue =
    searchContent.searchType === "COMPANY" && searchContent.searchValue
      ? searchContent.searchValue
      : "";
    ...
    <div className="flex flex-col">
      ...
      >
        <TextField
          label="Customer Name"
          onChange={(event) =>
            setSearchContent({ searchType: "CUSTOMER_NAME", searchValue: event.target.value })
          }
        ...
        <TextField
          label="month"
          onChange={(event) =>
            setSearchContent({ searchType: "MONTH", searchValue: event.target.value })
          }
        ...
        <TextField
          label="year"
          onChange={(event) =>
            setSearchContent({ searchType: "YEAR", searchValue: event.target.value })
          }
        ...
        <TextField
          label="plan"
          onChange={(event) =>
            setSearchContent({ searchType: "PLAN", searchValue: event.target.value })
          }
        ...
        <TextField
          label="company"
          onChange={(event) =>
            setSearchContent({ searchType: "COMPANY", searchValue: event.target.value })
          }
